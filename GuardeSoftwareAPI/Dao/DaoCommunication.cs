    ...
    public class CommunicationDao
    {
        ...
        private const string GET_COMMUNICATIONS_QUERY = @"
            SELECT 
                ...
                CASE
                    WHEN c.send_to_all_emails = 1 THEN 'Clientes y receptores externos de la base de datos'
                    ELSE ISNULL(
        ...
        public async Task<List<CommunicationDispatchDto>> GetDispatchesByCommunicationIdAsync(int communicationId)
        {
            ...
            string query = @"
                SELECT 
                    ...
                    CAST(0 AS BIT) AS IsExternalRecipient,
                ...
                SELECT
                    ...
                    CAST(1 AS BIT) AS IsExternalRecipient,
                    ISNULL(NULLIF(LTRIM(RTRIM(r.name)), ''), '(Sin nombre)') AS ClientName,
                    ISNULL(ch.name, 'Email') AS Channel,
                    ISNULL(d.status, CASE WHEN comm.status IN ('Failed', 'Finished w/ Errors') THEN 'Fallido' ELSE 'Pendiente' END) AS Status,
                    ISNULL(d.provider_response, CASE WHEN comm.status = 'Failed' THEN ISNULL(comm.error_message, 'Error en el envío') ELSE '' END) AS ErrorMessage,
                    ISNULL(FORMAT(d.dispatch_date, 'yyyy-MM-dd HH:mm'), '') AS DispatchDate,
                    d.recipient_phone AS RecipientPhone,
                ...
                LEFT JOIN communication_channel_content ccc
                   ...
                   AND EXISTS (
                       SELECT 1
                       FROM communication_channels channel_filter
                       WHERE channel_filter.channel_id = ccc.channel_id
                         AND channel_filter.name = 'Email'
                   )
                LEFT JOIN communication_channels ch ON ccc.channel_id = ch.channel_id
                LEFT JOIN (
                    ...
                    AND r.recipient_id = d.recipient_id
                    AND d.rn = 1
                WHERE (
                    (
                    ...
                    OR EXISTS (
                        SELECT 1
                        FROM mass_recipient_dispatches history_dispatch
                        JOIN communication_channel_content history_content
                            ON history_content.comm_channel_content_id = history_dispatch.comm_channel_content_id
                        WHERE history_content.communication_id = @Id
                          AND history_dispatch.recipient_id = r.recipient_id
                    )
                )
                ORDER BY ClientName, Channel;";
            ...
            foreach (DataRow row in table.Rows)
            {
                list.Add(new CommunicationDispatchDto
                {
                    ...
                    ExternalRecipientId = row["ExternalRecipientId"] is DBNull
                        ? null
                        : Convert.ToInt32(row["ExternalRecipientId"]),
                    IsExternalRecipient = row["IsExternalRecipient"] is not DBNull
                        && Convert.ToBoolean(row["IsExternalRecipient"]),
        ...
        {
            string query = @"
                INSERT INTO communications (creator_user_id, title, creation_date, scheduled_date, status, smtp_configuration_id, is_account_statement, is_next_month_statement, send_to_all_emails)
                OUTPUT INSERTED.communication_id
                VALUES (@CreatorUserId, @Title, GETDATE(), @ScheduledDate, @Status, @SmtpConfigId, @IsAccountStatement, @IsNextMonthStatement, @SendToAllEmails);";
            ...
            using (SqlCommand command = new(query, connection, transaction))
            {
                ...
                command.Parameters.AddWithValue("@SendToAllEmails", request.SendToAllEmails);
        ...
        public async Task<bool> InsertAllEmailRecipientsAsync(int communicationId, SqlConnection connection, SqlTransaction transaction)
        {
            string query = @"
                INSERT INTO communication_recipients (communication_id, client_id)
                SELECT DISTINCT @CommunicationId, e.client_id
                FROM emails e
                WHERE NULLIF(LTRIM(RTRIM(e.address)), '') IS NOT NULL";
        ...
        public async Task<List<RecipientForSendingDto>> GetRecipientsForSendingAsync(int communicationId, int commChannelContentId)
        {
            var recipients = new List<RecipientForSendingDto>();
            
            // El reintento se controla por canal. Si Email fue exitoso pero WhatsApp falló
            // (o viceversa), el segundo canal todavía debe poder procesarse.
            string query = @"
                SELECT DISTINCT
                    c.client_id AS Id,
                    c.full_name AS Name,
                    ...
                    (SELECT TOP 1 p.number
                     FROM phones p 
                     WHERE p.client_id = c.client_id AND p.whatsapp = 1 AND p.active = 1
                     ORDER BY p.phone_id) AS Phone,
                    (SELECT STRING_AGG(p.number, ';')
                     FROM phones p
                     WHERE p.client_id = c.client_id
                       ...
                       AND (
                           ...
                           OR ISNULL(comm.is_account_statement, 0) <> 1
                           OR NOT EXISTS (
                               SELECT 1
                               FROM dispatches d
                ...
                JOIN communications comm ON comm.communication_id = cr.communication_id
                JOIN communication_channel_content ccc
                    ON ccc.comm_channel_content_id = @CommChannelContentId
                   AND ccc.communication_id = comm.communication_id
                JOIN communication_channels channel ON channel.channel_id = ccc.channel_id
                WHERE cr.communication_id = @Id AND cr.is_selected_for_retry = 1 AND c.active = 1 AND c.receive_communications = 1
                AND (
                    (
                        channel.name = 'WhatsApp'
                        AND ISNULL(comm.is_account_statement, 0) = 1
                        AND (
                            NOT EXISTS (
                                SELECT 1
                                FROM phones p
                    ...
                    OR (
                        (channel.name <> 'WhatsApp' OR ISNULL(comm.is_account_statement, 0) <> 1)
                        AND NOT EXISTS (
                            SELECT 1
                            FROM dispatches d
                            WHERE d.comm_channel_content_id = @CommChannelContentId
            ...
            foreach (DataRow row in table.Rows)
            {
                var whatsappPhones = row["WhatsAppPhones"] is DBNull
                    ? new List<string>()
                    : (row["WhatsAppPhones"].ToString() ?? string.Empty)
        ...
        public async Task<List<RecipientForSendingDto>> GetAllEmailRecipientsForSendingAsync(int communicationId, int commChannelContentId)
        {
            var recipients = new List<RecipientForSendingDto>();

            // No se filtra por emails.active, clients.active ni receive_communications:
            // esta modalidad representa todos los registros cargados en emails y en la
            // agenda de receptores externos. Los receptores externos solo participan en
            // comunicados generales por email, nunca en estados de cuenta.
            string query = @"
                WITH ClientRecipients AS (
                    ...
                    WHERE EXISTS (
                        ...
                        WHERE comm.communication_id = @Id
            ...
            DataTable table = await _accessDB.GetTableAsync("AllEmailRecipients", query, parameters);
        ...
        {
            const string query = @"
                SELECT ISNULL(send_to_all_emails, 0) AS SendToAllEmails
                FROM communications
                WHERE communication_id = @Id";

            var table = await _accessDB.GetTableAsync(
                "CommunicationEmailScope",
                query,
                new[] { new SqlParameter("@Id", communicationId) });

            return table.Rows.Count > 0
                && table.Rows[0]["SendToAllEmails"] is not DBNull
                && Convert.ToBoolean(table.Rows[0]["SendToAllEmails"]);
        }

        public async Task UpdateRecipientsRetrySelectionAsync(
            int communicationId,
            ...
            List<int>? selectedExternalRecipientIds = null)
        {
            string resetQuery = @"
                ...
                SET is_selected_for_retry = CASE
                    WHEN EXISTS (
                        ...
                        FROM communication_channel_content ccc
                        JOIN communication_channels channel ON channel.channel_id = ccc.channel_id
                        WHERE ccc.communication_id = cr.communication_id
                          AND (
                              (
                                  channel.name = 'WhatsApp'
                                  AND ISNULL(comm.is_account_statement, 0) = 1
                                  AND (
                                      NOT EXISTS (
                                          SELECT 1
                                          FROM phones p
                                          WHERE p.client_id = cr.client_id
                                            AND p.whatsapp = 1
                                            AND p.active = 1
                                      )
                                      OR EXISTS (
            ...
            // Los receptores externos no tienen fila en communication_recipients porque
            // esa tabla exige un client_id real. Su selección de reintento se guarda en
            // el último intento de mass_recipient_dispatches.
            string externalRetryQuery;
            if (selectedExternalRecipientIds is null)
            {
                externalRetryQuery = @"
                    UPDATE d
                    SET is_selected_for_retry = 1
                    FROM mass_recipient_dispatches d
                    JOIN communication_channel_content ccc
            ...
            {
                var externalIds = selectedExternalRecipientIds
                    .Where(id => id > 0)
                    .Distinct()
                    .ToList();
                string externalIdList = externalIds.Count == 0
                    ? "0"
                    : string.Join(",", externalIds);

                externalRetryQuery = $@"
                    UPDATE d
                    SET is_selected_for_retry =
                        CASE WHEN d.recipient_id IN ({externalIdList}) THEN 1 ELSE 0 END
                    FROM mass_recipient_dispatches d
                    JOIN communication_channel_content ccc
                        ON ccc.comm_channel_content_id = d.comm_channel_content_id
                    WHERE ccc.communication_id = @Id
        ...
        public async Task LogSendAttemptAsync(
            int idCommChannelContent,
            int idCliente,
            string status,
            string response,
        ...
        public async Task LogMassRecipientSendAttemptAsync(
            int idCommChannelContent,
            int recipientId,
            string status,
            string response,
        ...
        public async Task<string?> GetDispatchContentAsync(int dispatchId)
        {
            ...
            if (dispatchId < 0)
            {
                query = "SELECT sent_content FROM mass_recipient_dispatches WHERE dispatch_id = @Id";
                lookupId = -dispatchId;
            }
            else
            {
                query = "SELECT sent_content FROM dispatches WHERE dispatch_id = @Id";
                lookupId = dispatchId;
            }

            var result = await _accessDB.ExecuteScalarAsync(
                query,
                new[] { new SqlParameter("@Id", lookupId) });
        ...
        public async Task<List<ClientRecipientDto>> GetClientsForSelectorAsync()
        {
            ...
            string query = @"
                SELECT 
                    ...
                    ISNULL((SELECT STRING_AGG(NULLIF(LTRIM(RTRIM(address)), ''), ';')
                            FROM emails
                            WHERE client_id = c.client_id AND active = 1), '') AS Email,
        ...
        public async Task<bool> IsTestCommunicationAsync(int communicationId)
        {
            const string q = @"
                SELECT CASE
                    WHEN LEFT(LTRIM(RTRIM(title)), 8) = '[PRUEBA]' THEN 1
                    ELSE 0
                END
                FROM communications
                WHERE communication_id = @Id";
